...
mkAwsCreds =
  AwsCreds
  { awsK = "AKIAI6GDZ5ELIC7ABKJA"
  , awsS = "wsuBXNMeGs2Ty7qNNMhxgeFXqDs1Nwxb8NnzLzXL"
  }

canonicalRequest :: Request -> ByteString -> ByteString
canonicalRequest req bbody =
  C.concat $
  intersperse
    "n"
    [ method req
...
headers req =
  sortBy
    (\(a, _) (b, _) -> compare a b)
    (("host", host req) : requestHeaders req)
...
v4DerivedKey
  ...
  -> ByteString
v4DerivedKey bsecretAccessKey date bregion service =
  hmacSHA256 kService "aws4_request"
  where
    kDate = hmacSHA256 ("AWS4" <> bsecretAccessKey) date
...
stringToSign
  ...
  -> ByteString
stringToSign date bregion service hashConReq =
  C.concat
    [ "AWS4-HMAC-SHA256n"
...
createSignature req bbody now key bregion = v4Signature dKey toSign
  where
    canReqHash = hexHash $ canonicalRequest req bbody
...
sendEmail sendReq = do
  fReq <-
    parseUrlThrow $
    "https://email." ++ C.unpack (region sendReq) ++ ".amazonaws.com"
  now <- getCurrentTime
...
authenticateRequest sendReq now req bbody =
  ...
  where
    sig =
      createSignature req bbody now (secretAccessKey sendReq) (region sendReq)

toAddressQuery :: [ByteString] -> SimpleQuery
toAddressQuery tos =
  zipWith
    (\index address ->
       ("Destination.ToAddresses.member." <> C.pack (show index), address))
    ([1 ..] :: [Int])
    tos
...
sesMail bto bsub bcontent = do
  ...
  case response of
    ...
    Right resp ->
      putStrLn $
      "Successfully sent with message ID : " ++ unpack (messageId resp)
